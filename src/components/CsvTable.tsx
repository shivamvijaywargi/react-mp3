...
interface IProps {
  tableDatas: (IData | string)[][] | string[][];
}
...
export default function CsvTable({ tableDatas }: IProps) {
  ...
  const rows = tableDatas.map((tableData) =>
    createData(
      parseInt(tableData[0]),
      tableData[1],
      parseInt(tableData[2]),
      parseInt(tableData[3])
    )
  );
